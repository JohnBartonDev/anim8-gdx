...
public class Anim8Gui extends InputAdapter {
	...
	private Consumer<Array<Pixmap>> customWriteConsumer;
	...
	public void stop() {
		if(customWriteConsumer != null) {
			customWriteConsumer.accept(frames);
		}
		else {
//			try(BufferedOutputStream bos = new BufferedOutputStream(new))
	...
	public void setCustomWriteConsumer(Consumer<Array<Pixmap>> consumer) {
		customWriteConsumer = consumer;
	}
...
//		if(batch.isDrawing()) batch.end();
		...
		oldProj = batch.getProjectionMatrix();
		...
		shapeDrawer.getBatch().setProjectionMatrix(oldProj);
	}
